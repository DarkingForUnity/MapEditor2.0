...
//    }
    ...
    /// <summary>
    /// Calculate the game objest's depth based on the sprite's layerID and sorting order.
    /// All sprite's sorting order must be less than 9999.
    /// Create by Quibos
    /// </summary>

    static public int CalculateSpriteRaycastDepth ( GameObject go ) {

        SpriteRenderer sprite = go.GetComponent<SpriteRenderer>( );
        if ( sprite != null ) {
            if ( sprite.sortingOrder >= 10000 ) {
                Debug.LogWarning( "All sprite's sorting order that used to NGUI event system must be less than 9999." );
            }
            return sprite.sortingLayerID * 10000 + sprite.sortingOrder;
        }
        ...
        for ( int i = 0 , imax = sprites.Length; i < imax; ++i ) {
            if ( sprites[i].enabled ) {
                if ( sprites[i].sortingOrder >= 10000 ) {
                    Debug.LogWarning( "All sprite's sorting order that used to NGUI event system must be less than 9999." );
                }
                depth = Mathf.Min( depth , sprites[i].sortingLayerID * 10000 + sprites[i].sortingOrder );
            }
        }
        return depth;
    }
