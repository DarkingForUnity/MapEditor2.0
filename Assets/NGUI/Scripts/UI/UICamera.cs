...
public class UICamera : MonoBehaviour
{
    ...
    /// <summary>
    /// The object hit by the current Raycast that was the result of a mouse or touch event.
    /// </summary>

    static public GameObject currentObject {
        get {
            UICamera cam = UICamera.mainCamera.GetComponent<UICamera>( );
            if ( cam == null ) return null;
            if ( cam.useTouch ) cam.ProcessTouches( );
            else if ( cam.useMouse ) cam.ProcessMouse( );
	...
	static public bool Raycast (Vector3 inPos)
	{
		for (int i = 0; i < list.size; ++i)
		{
			...
			else if (cam.eventType == EventType.UI_2D)
			{
				...
				continue;
			} else if ( cam.eventType == EventType.Sprite_2D ) {
                if ( m2DPlane.Raycast( ray , out dist ) ) {
                    lastWorldPosition = ray.GetPoint( dist );
                    Collider2D[ ] hits = Physics2D.OverlapPointAll( lastWorldPosition , mask );
                    ...
                    if ( hits.Length > 1 ) {
                        for ( int b = 0; b < hits.Length; ++b ) {
                            ...
                            mHit.depth = NGUITools.CalculateSpriteRaycastDepth( go );
